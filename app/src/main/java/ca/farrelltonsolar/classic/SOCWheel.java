 ...
 */
public class SOCWheel extends View {
    ...
    public SOCWheel(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void setupBounds() {
        ...
        // Add the offset
        // Add the offset
        paddingTop = 2 + (yOffset / 2);
        paddingBottom = 2 + (yOffset / 2);
        paddingLeft = 2 + (xOffset / 2);
        paddingRight = 2 + (xOffset / 2);

        rectBounds = new RectF(paddingLeft,
                paddingTop,
                layout_width - paddingRight,
                layout_height - paddingBottom);

        circleBounds = new RectF(paddingLeft + rimWidth,
                paddingTop + rimWidth,
                layout_width - paddingRight - rimWidth,
                layout_height - paddingBottom - rimWidth);
        ...
        fullRadius = (minValue - paddingRight - rimWidth) / 2;
        circleRadius = (fullRadius - rimWidth) + 1;
    }
...
//        setText(Math.round(((float) progress / 360) * 100) + "%");
     ...
     */
    public void setProgress(int socVal) {
        if (socVal < 20) {
            setBarColor(Color.RED);
        }
        else if (socVal < 40) {
            setBarColor(Color.rgb(255, 200, 0));
        }
        else if (socVal < 60) {
